...
static inline void
ptr_device_memory_reset(ptr_device_t * device)
{
    # pragma message(TODO "This is leaking")
    ptr_alloc_chunk_t * chunk0 = (ptr_alloc_chunk_t *) malloc(sizeof(ptr_alloc_chunk_t));
    assert(chunk0);
    memcpy(chunk0, &(device->memdev.chunk0), sizeof(ptr_alloc_chunk_t));
...
ptr_device_memory_set_chunk0(
    ...
    device->memdev.chunk0.state         = PTR_ALLOC_CHUNK_STATE_FREE;
    ...
    ptr_device_memory_reset(device);
}

static inline void
ptr_device_init_memory(ptr_device_t * device)
{
    PTR_MUTEX_INIT(device->memdev.lock);
    ptr_device_memory_set_chunk0(device, 0, 0);
}

void
ptr_memory_deallocate(
    ptr_device_t * device,
    ptr_alloc_chunk_t * chunk
) {

    bool delete_chunk = false;

    PTR_MUTEX_LOCK(device->memdev.lock);
    {
        chunk->state = PTR_ALLOC_CHUNK_STATE_FREE;
        chunk->use_counter = 0;

        /* can we merge chunk into next_chunk ? */
        ptr_alloc_chunk_t * next_chunk = chunk->next;
        if (next_chunk && next_chunk->state == PTR_ALLOC_CHUNK_STATE_FREE)
        {
        ...
        if (prev_chunk)
        {
            ...
            else if (!delete_chunk)
            {
                ...
                while (prev_chunk && prev_chunk->state != PTR_ALLOC_CHUNK_STATE_FREE)
    ...
    PTR_MUTEX_UNLOCK(device->memdev.lock);
...
ptr_memory_allocate(
    ...
    /* best fit strategy */
    ptr_alloc_chunk_t * curr = device->memdev.free_chunk_list;
    ptr_alloc_chunk_t * prevfree = NULL;
    size_t min_size = 0;
    ptr_alloc_chunk_t * min_size_curr = NULL;
    ptr_alloc_chunk_t * min_size_prevfree = NULL;
    ...
    {
        size_t curr_size = curr->size;
        ptr_alloc_chunk_t * remainder = (ptr_alloc_chunk_t *) malloc(sizeof(ptr_alloc_chunk_t));
        remainder->device_ptr   = size + curr->device_ptr;
        remainder->size         = (curr_size - size);
        remainder->state        = PTR_ALLOC_CHUNK_STATE_FREE;
    ...
    if (curr != NULL)
    {
        ...
        curr->state = PTR_ALLOC_CHUNK_STATE_ALLOCATED;
        curr->freelink = NULL;
    }
...
ptr_memory_deallocate_all(void)
{
    ...
    for (ptr_device_global_id_t device_global_id = 0 ;
            ...
            ++device_global_id)
    {
        ptr_device_t * device = ptr_device_get(device_global_id);
        assert(device);
...
ptr_device_commit(
    ...
    assert(device->state == PTR_DEVICE_STATE_INIT);
    device->state = PTR_DEVICE_STATE_COMMIT;
}
...
ptr_device_init(
    ...
    assert(device->state == PTR_DEVICE_STATE_CREATE);
    device->state = PTR_DEVICE_STATE_INIT;
}
...
ptr_device_create(
    ...
    if (drivers->devices.n == PTR_DEVICES_MAX)
        LOGGER_FATAL("Too many devices. Increase 'PTR_DEVICES_MAX' and recompile Xkblas");

    ptr_driver_t * driver = drivers->list + driver_id;
    ptr_device_t * device = driver->f_device_create(driver, device_driver_id);
    assert(device);

    device->state     = PTR_DEVICE_STATE_CREATE;
...
/////////////////////////
int
ptr_device_poll(ptr_device_t * device)
{
    int err = 0;
    assert(ThreadWorker::self() == device->thread);

    err = device->offloader.launch_ready_instructions(PTR_STREAM_TYPE_ALL);
    assert( (err == 0) || (err == EINPROGRESS));

    err = device->offloader.progress_pending_instructions(PTR_STREAM_TYPE_ALL, false);
    assert( (err == 0) || (err == EINPROGRESS));
...
static inline int
ptr_device_accept_new_task(ptr_device_t * device)
{
...
ptr_device_prepare_task(
    ...
    if (task->fetched() == TASK_STATE_DATA_FETCHED)
    {
        ...
        ptr_device_task_execute(device, task);
    }
    else
    {
...
ptr_device_progress(
    ...
    int err = ptr_device_poll(device);
    assert((err == 0) || (err == EINPROGRESS));
}

/* main loop for the thread responsible the passed device */
static inline int
ptr_device_thread_main_loop(
    ptr_driver_t * driver,
    ptr_device_t * device
) {
    assert(ThreadWorker::self() == device->thread);
    assert(device->state == PTR_DEVICE_STATE_COMMIT);
    device->state = PTR_DEVICE_STATE_RUNNING;

    # pragma message(TODO "do we really need this mem_barrier here?")
    mem_barrier();

    ThreadWorker * worker = ThreadWorker::self();
    while (device->state == PTR_DEVICE_STATE_RUNNING)
    {
        Task * task;
        while ((task = worker->pop()) == NULL && device->offloader.is_empty(PTR_STREAM_TYPE_ALL))
            worker->pause();

        ptr_device_progress(driver, device);
        if (task)
            ptr_device_prepare_task(driver, device, task);
    }
...
void *
ptr_device_thread_main(void * a)
{
    # pragma message(TODO "Implement device thread")

    ptr_driver_device_thread_arg_t * arg = (ptr_driver_device_thread_arg_t *) a;
    ptr_drivers_t * drivers  = arg->drivers;
    ...
    /* init the device */
    ptr_device_t * device = ptr_device_create(drivers, driver_id, device_driver_id);
    ptr_device_init(driver, device);
    ...
    /* */
#if PTR_SLEEP_DEVICETHREAD
    ptr_fifo_register_waiter( device->ld->queue, (void (*)(void *))ptr_offload_device_wakeup, device );
#else
    ptr_fifo_register_waiter( device->ld->queue, 0, 0);
#endif

    /* infinite loop with the device context */
    int err = ptr_sched_idle_offload(thread, _ptr_device_finalize, device);
    assert((err==0)||(err==eintr));

    /* thread is stopped */
    assert(0 == pthread_mutex_lock(&device->lock));
    device->state = PTR_DEVICE_STATE_STOPPED;
    assert(0 == pthread_cond_signal(&device->cond_sleep));
    assert(0 == pthread_mutex_unlock(&device->lock));

    assert(0 == pthread_mutex_lock(&device->lock));
    _ptr_offload_device_finalize(device);
    ptr_offload_device_pop( device );
    ptr_localitydomain_destroy(device->ld);
    device->state = PTR_DEVICE_STATE_FINALIZED;
    ...
    assert(0 == pthread_mutex_unlock(&device->lock));
    ptr_thread_unbind(thread);
    _ptr_self_context = 0;
    device->state = PTR_DEVICE_STATE_DESTROYED;
