...
typedef struct  ptr_stream_hip_t
{
    ptr_stream_t super;
...
typedef struct  ptr_device_hip_t
{
...
static ptr_device_hip_t DEVICES[PTR_DEVICES_MAX];
...
static inline ptr_device_hip_t *
__get_device_hip(int device_id)
{
    return (ptr_device_hip_t *) __get_device(device_id);
}

/* Convert ptr driver device id (in [0..ngpus-1]) to the hip driver device id (in [0, INT_MAX[) */
static int CUDA_DEVICE_ID[PTR_DEVICES_MAX];
...
uint64_t hip_get_free_mem(int device_id)
{
    ...
    ptr_device_hip_t * device = __get_device_hip(device_id);
...
static int hip_perf_topo[PTR_DEVICES_MAX][PTR_DEVICES_MAX];
...
PTR_DRIVER_ENTRYPOINT(init)(void)
{
    ...
    # if 0
    int nrrl = getenv("PTR_RRL_SIZE") ? atoi(getenv("PTR_RRL_SIZE")) : 1;
    if (nrrl <= 0)
        nrrl = 1;
    ...
    {
        ptr_hip_init_reqreg_list(&all_rrl[i]);
        int err = pthread_create(&all_rrl[i].thread, 0, ptr_hip_register_thread, (void*)(uintptr_t)i );
        assert(err ==0);
    }
    # endif
...
PTR_DRIVER_ENTRYPOINT(device_set_cpuset)(cpu_set_t * schedset, int device_id)
{
    ...
    assert(device_id >= 0);
    assert(device_id < PTR_DEVICES_MAX);
...
{
    assert(INITIALIZED);
    assert(device_id >= 0 && device_id < PTR_DEVICES_MAX);

    ptr_device_hip_t * device = DEVICES + device_id;
...
static int
PTR_DRIVER_ENTRYPOINT(get_source)(int dst_global_id, int bitmask)
{
    ...
    int dst_driver_id = -1;
    for( int driver_dev_id = 0; driver_dev_id < PTR_DEVICES_MAX; driver_dev_id++ )
    {
         ptr_device_hip_t* pdev = DEVICES + driver_dev_id;
         if( pdev->inherited.state == PTR_DEVICE_STATE_CREATE )
             continue;
    ...
    for( int driver_dev_id = 0; driver_dev_id < PTR_DEVICES_MAX; driver_dev_id++ )
    {
         ...
         if( !(pdev->inherited.state == PTR_DEVICE_STATE_COMMIT || pdev->inherited.state == PTR_DEVICE_STATE_RUNNING) )
...
# endif /* NDEBUG */
    ...
    for (int dev = 0 ; dev < PTR_DEVICES_MAX ; ++dev)
    {
        ...
        {
            # if 0
            device->affinity[0] |= (1UL<<ptr_memory_asid_get_lid(ptr_device_list[j]->inherited.memdev.asid));
            ld->affinity[0] |= (1UL<<ptr_memory_asid_get_lid(ptr_device_list[j]->inherited.memdev.asid));
            # endif
        }
        else
        {
            ptr_device_hip_t * odevice = __get_device_hip(j);
            if (odevice->inherited.state != PTR_DEVICE_STATE_INIT)
                continue ;
            ...
            if (access)
            {
                ...
                if ((res == hipSuccess) || (res == hipErrorPeerAccessAlreadyEnabled))
                {
                    ...
                    {
                        # if 0
                        device->affinity[rank-1] |= (1UL <<ptr_memory_asid_get_lid(ptr_device_list[j]->inherited.memdev.asid));
                        # endif
                    }
                }
                else
...
PTR_DRIVER_ENTRYPOINT(stream_instruction_launch)(
    ...
    ptr_stream_hip_t * stream = (ptr_stream_hip_t *) istream;
    assert(stream);

    switch (instr->type)
    {
        case (PTR_STREAM_INSTR_TYPE_NOP):
        {
            return 0;
        }
...
PTR_DRIVER_ENTRYPOINT(stream_create)(
    ...
    uint8_t * mem = (uint8_t *) malloc(sizeof(ptr_stream_hip_t) + capacity * sizeof(hipEvent_t));
    assert(mem);

    ptr_stream_hip_t * stream = (ptr_stream_hip_t *) mem;
    ...
    /* events */
    stream->hip.events.end = (hipEvent_t *) (mem + sizeof(ptr_stream_hip_t));
