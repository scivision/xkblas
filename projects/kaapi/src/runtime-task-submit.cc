...
void
kaapi_runtime_submit_task(kaapi_runtime_t * context, Task * task)
{
    ...
    kaapi_device_global_id_t device_id = UNSPECIFIED_DEVICE_GLOBAL_ID;
    ...
    if (task->ocr_access_index != UNSPECIFIED_TASK_ACCESS)
    {
        ...
        const kaapi_device_global_id_bitfield_t owners = memtree->who_owns(access);
        if (owners)
            device_id = (kaapi_device_global_id_t) __random_set_bit(owners) - 1;
    }
