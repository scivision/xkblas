...
Offloader::init(
    ...
    uint16_t cnt = 0;
    for (int stype = 0 ; stype < KAAPI_STREAM_TYPE_ALL ; ++stype)
    {
    ...
    kaapi_stream_t ** all_streams = (kaapi_stream_t **) malloc(sizeof(kaapi_stream_t *) * cnt);
    assert(all_streams);
    ...
    for (int stype = 0 ; stype < KAAPI_STREAM_TYPE_ALL ; ++stype)
    {
        ...
        {
            all_streams[i] = f_stream_create(static_cast<kaapi_stream_type_t>(stype), conf->capacity);
...
bool
Offloader::is_empty(kaapi_stream_type_t stype) const
{
    int err = 0;

    unsigned int bgn = (stype == KAAPI_STREAM_TYPE_ALL) ?                      0 : stype;
    unsigned int end = (stype == KAAPI_STREAM_TYPE_ALL) ? KAAPI_STREAM_TYPE_ALL : stype + 1;
...
int
Offloader::launch_ready_instructions(kaapi_stream_type_t stype)
{
    ...
    for (unsigned int s = bgn ; s < end ; ++s)
    {
        for (unsigned int i = 0 ; i < this->count[s] ; ++i)
        {
            kaapi_stream_t * stream = this->streams[s][i];
            assert(stream);
...
int
Offloader::progress_pending_instructions(kaapi_stream_type_t stype, bool blocking)
{
    # pragma message(TODO "Better handling of error in case 'STREAM_ALL'")
    ...
    for (unsigned int s = bgn ; s < end ; ++s)
    {
        for (unsigned int i = 0 ; i < this->count[s] ; ++i)
        {
            ...
            kaapi_stream_instruction_counter_t n;
            do {
                stream->lock();
                err = stream->progress_pending_instructions(blocking);
                stream->unlock();
                n = stream->pending.size();
            } while (s == KAAPI_STREAM_TYPE_KERN && n > ctx->conf.device.offloader.streams[KAAPI_STREAM_TYPE_KERN].concurrency);
...
kaapi_stream_t *
Offloader::stream_next(kaapi_stream_type_t stype)
{
...
Offloader::instruction_new(
    const kaapi_stream_type_t stype,               /* IN  */
          kaapi_stream_t ** pstream,               /* OUT */
    const kaapi_stream_instruction_type_t itype,   /* IN  */
          kaapi_stream_instruction_t ** pinstr,    /* OUT */
    ...
    kaapi_stream_t * stream = this->stream_next(stype);
...
try_instruction_new:
    ...
    do {
        ...
        LOGGER_FATAL("Stream is full, increase 'KAAPI_OFFLOADER_CAPACITY' or implement support for full-queue management in PTR yourself :-) (sorry)");
