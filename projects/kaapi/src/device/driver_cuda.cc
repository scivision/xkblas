...
# define KAAPI_DRIVER_ENTRYPOINT(N) KAAPI_DRIVER_TYPE_CUDA_ ## N
...
typedef struct  kaapi_stream_cuda_t
{
    kaapi_stream_t super;
...
typedef struct  kaapi_device_cuda_t
{
    ...
    kaapi_device_global_id_bitfield_t * affinity;
...
static kaapi_device_cuda_t DEVICES[KAAPI_DEVICES_MAX];

static inline kaapi_device_t *
__get_device(int device_driver_id)
{
    return (kaapi_device_t *) (DEVICES + device_driver_id);
}

static inline kaapi_device_cuda_t *
__get_device_cuda(int device_driver_id)
{
    return (kaapi_device_cuda_t *) __get_device(device_driver_id);
}

/* Convert ptr driver device id (in [0..ngpus-1]) to the cuda driver device id (in [0, INT_MAX[) */
static int DEVICE_CUDA_ID[KAAPI_DEVICES_MAX];
...
uint64_t cuda_get_free_mem(int device_driver_id)
{
    ...
    kaapi_device_cuda_t * device = __get_device_cuda(device_driver_id);
...
KAAPI_DRIVER_ENTRYPOINT(init)(void)
{
    ...
    KAAPI_MUTEX_LOCK(DRIVER_MUTEX);
    {
        ...
        for (int i = 0 ; i < KAAPI_DEVICES_MAX ; ++i)
        {
            kaapi_device_cuda_t * device = __get_device_cuda(i);
            assert(device);
            device->inherited.state = KAAPI_DEVICE_STATE_DEALLOCATED;
        }
        ...
        uint32_t ngpus = MIN(KAAPI_DRIVER_ENTRYPOINT(get_ndevices_max)(), context->conf.ngpus);
...
static int
KAAPI_DRIVER_ENTRYPOINT(device_set_cpuset)(cpu_set_t * schedset, int device_driver_id)
{
    if (schedset == NULL)
        return EINVAL;

    assert(device_driver_id >= 0);
    assert(device_driver_id < KAAPI_DEVICES_MAX);
...
KAAPI_DRIVER_ENTRYPOINT(device_create)(kaapi_driver_t * driver, int device_driver_id)
{
    assert(INITIALIZED);
    assert(device_driver_id >= 0 && device_driver_id < KAAPI_DEVICES_MAX);

    kaapi_device_cuda_t * device = __get_device_cuda(device_driver_id);
    assert(device);
    assert(device->inherited.state == KAAPI_DEVICE_STATE_DEALLOCATED);

    return (kaapi_device_t *) device;
}

static void
KAAPI_DRIVER_ENTRYPOINT(device_init)(int device_driver_id)
{
    assert(INITIALIZED);

    kaapi_device_cuda_t * device = __get_device_cuda(device_driver_id);
    assert(device);
    assert(device->inherited.state == KAAPI_DEVICE_STATE_CREATE);
    ...
    __check_error(res);
    kaapi_device_memory_set_chunk0(&(device->inherited), device_ptr, size);
}

static int
KAAPI_DRIVER_ENTRYPOINT(device_destroy)(kaapi_device_t * device)
{
    device->state = KAAPI_DEVICE_STATE_DESTROY;
    free(device);
    return 0;
}

static int
KAAPI_DRIVER_ENTRYPOINT(device_attach)(int device_driver_id)
{
    assert(INITIALIZED);
 ...
 */
static kaapi_device_global_id_t
KAAPI_DRIVER_ENTRYPOINT(get_source)(
    kaapi_device_global_id_t dst_global_id,
    kaapi_device_global_id_bitfield_t bitfield
) {
    ...
    /* retrieve dst device */
    kaapi_device_cuda_t * device = (kaapi_device_cuda_t *) kaapi_device_get(dst_global_id);
    assert(device);
    ...
    for (int rank = 0 ; rank < cuda_count_perfrank -1 ; ++rank)
    {
        ...
        const kaapi_device_global_id_bitfield_t mask = bitfield & device->affinity[rank];
        if (mask == 0)
            continue ;
...
static int
KAAPI_DRIVER_ENTRYPOINT(device_commit)(int device_driver_id)
{
# ifndef NDEBUG
    int cu_devid = -1;
...
# endif /* NDEBUG */
    ...
    kaapi_device_cuda_t * device = __get_device_cuda(device_driver_id);
    assert(device);
    assert(device->inherited.state == KAAPI_DEVICE_STATE_INIT);

    int device_cuda_id = __get_device_cuda_id(device_driver_id);

    const uint64_t size = sizeof(kaapi_device_global_id_bitfield_t) * cuda_count_perfrank;
    device->affinity = (kaapi_device_global_id_bitfield_t *) malloc(size);
    memset(device->affinity, 0, size);

    /* all other devices have been initialized, enable peer */
    for (int other_device_driver_id = 0 ; other_device_driver_id < KAAPI_DEVICES_MAX ; ++other_device_driver_id)
    {
        kaapi_device_cuda_t * other_device = __get_device_cuda(other_device_driver_id);
        assert(other_device);
        if (other_device->inherited.state < KAAPI_DEVICE_STATE_INIT)
            continue ;
        ...
        {
            device->affinity[0] |= (kaapi_device_global_id_bitfield_t) (1UL << other_device->inherited.global_id);
        }
        else
        {
            int access;
            ...
            if (access)
            {
                ...
                if ((res == cudaSuccess) || (res == cudaErrorPeerAccessAlreadyEnabled))
                {
                    ...
                    {
                        device->affinity[rank - 1] |= (kaapi_device_global_id_bitfield_t) (1UL << other_device_cuda_id);
                    }
                }
                else
                {
...
static int
KAAPI_DRIVER_ENTRYPOINT(memory_register)(
    void * ptr,
    uint64_t size
) {
...
static int
KAAPI_DRIVER_ENTRYPOINT(memory_unregister)(
    void * ptr,
    uint64_t size
) {
    (void) size;
...
KAAPI_DRIVER_ENTRYPOINT(stream_instruction_launch)(
    ...
    kaapi_stream_cuda_t * stream = (kaapi_stream_cuda_t *) istream;
    assert(stream);
    ...
    switch (instr->type)
    {
        ...
        {
            assert(istream->type == KAAPI_STREAM_TYPE_KERN);
            assert(stream->cu.handle.high);
            assert(stream->cu.blas.handle);

            kaapi_stream_instruction_kernel_t * op = &instr->kern;
            task_launcher_t launcher = {
                .task   = op->task,
                .target = KAAPI_DRIVER_TYPE_CUDA,
                .handle = stream->cu.blas.handle
            };
            kaapi_task_launch(&launcher);

            # pragma message(TODO "Add support for end event records")

            kaapi_stream_instruction_counter_t wp = istream->pending.pos.w % istream->pending.capacity;
...
cuda_stream_instructions_progress(
    ...
    kaapi_stream_instruction_counter_t     size = istream->pending.pos.w - istream->pending.pos.r;
    kaapi_stream_instruction_counter_t      okp = istream->ok_p;
    int64_t                             prev_okp = ((int64_t) okp) - 1;
    assert(prev_okp < (int64_t) okp);

    while (okp < istream->pending.pos.w)
    {
        const kaapi_stream_instruction_counter_t idx = okp % istream->pending.capacity;
        kaapi_stream_instruction_t * instr = istream->pending.instr + idx;
    ...
    {
        istream->ok_p = (kaapi_stream_instruction_counter_t) (prev_okp + 1);
        return 0;
    }
...
KAAPI_DRIVER_ENTRYPOINT(stream_instructions_progress)(
    ...
    for (kaapi_stream_instruction_counter_t p = istream->pending.pos.r ; p < istream->ok_p ; ++p)
        istream->complete(p);
...
KAAPI_DRIVER_ENTRYPOINT(stream_create)(
    ...
    uint8_t * mem = (uint8_t *) malloc(sizeof(kaapi_stream_cuda_t) + capacity * sizeof(cudaEvent_t));
    assert(mem);

    kaapi_stream_cuda_t * stream = (kaapi_stream_cuda_t *) mem;
    ...
    /* events */
    stream->cu.events.end = (cudaEvent_t *) (mem + sizeof(kaapi_stream_cuda_t));
    ...
    if (type == KAAPI_STREAM_TYPE_KERN)
    {
        cublasStatus_t cres = cublasCreate(&stream->cu.blas.handle);
        kaapi_cublas_status_check(cres);
...
const char *
KAAPI_DRIVER_ENTRYPOINT(device_info)(int device_driver_id)
{
