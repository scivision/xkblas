...
static void
kaapi_driver_init(kaapi_drivers_t * drivers, uint8_t driver_id, uint8_t ngpus)
{
    ...
    for (uint8_t i = 0; i < n_devices; ++i)
    {
        ...
        kaapi_driver_device_thread_arg_t * arg = (kaapi_driver_device_thread_arg_t *) malloc(sizeof(kaapi_driver_device_thread_arg_t));
        arg->drivers = drivers;
        arg->driver_id = driver_id;
        arg->device_driver_id = i;

        pthread_t thread;
        err = pthread_create(&thread, &attr, kaapi_device_thread_main, arg);
        if (err)
        {
...
void
kaapi_drivers_init(kaapi_drivers_t * drivers, uint8_t ngpus)
{
    ...
    // LOAD DRIVERS
    void (*loaders[KAAPI_DRIVER_TYPE_MAX])(kaapi_driver_t *);
    memset(loaders, 0, sizeof(loaders));

# if USE_CPU
    extern void KAAPI_DRIVER_CPU_get_driver(kaapi_driver_t *);
    loaders[KAAPI_DRIVER_CPU] = KAAPI_DRIVER_CPU_get_driver;
# endif /* USE_CPU */

# if USE_CUDA
    extern void KAAPI_DRIVER_TYPE_CUDA_get_driver(kaapi_driver_t *);
    loaders[KAAPI_DRIVER_TYPE_CUDA] = KAAPI_DRIVER_TYPE_CUDA_get_driver;
# endif /* USE_CUDA */

    uint8_t i;
    for (i = 0 ; i < KAAPI_DRIVER_TYPE_MAX && drivers->devices.n < ngpus ; ++i)
    {
        void (*loader)(kaapi_driver_t *) = loaders[i];
        if (loader)
        {
            loader(drivers->list + i);
            kaapi_driver_init(drivers, i, ngpus);
    ...
    for (i = 0 ; i < KAAPI_DRIVER_TYPE_MAX ; ++i)
    {
        kaapi_driver_t * driver = drivers->list + i;
...
void
kaapi_drivers_deinit(kaapi_drivers_t * drivers)
{
    # pragma message(TODO "Implement driver_deinit - synchronize all devices threads")
}

kaapi_device_t *
kaapi_get_device_host(kaapi_drivers_t * drivers)
{
...
int
kaapi_task_launch(task_launcher_t * launcher)
{
    assert(launcher);
    assert(launcher->task);
    assert(launcher->task->fmtid);
    assert(launcher->target >= 0 && launcher->target <= KAAPI_DRIVER_TYPE_MAX);
...
static void
kaapi_device_task_executed_callback(
    const void * args[KAAPI_CALLBACK_ARGS_MAX]
) {
    assert(args[0]);
    kaapi_device_task_executed((Task *) args[0]);
}
 ...
 */
void
kaapi_device_task_execute(
    kaapi_device_t * device,
               Task * task
) {
    assert(KAAPI_CALLBACK_ARGS_MAX >= 1);

    /* running an empty task */
    if (task->fmtid == TASK_FORMAT_NULL)
    {
        kaapi_device_task_executed(task);
    }
    else
    {
        ...
        else
        {
            ...
            callback.func    = kaapi_device_task_executed_callback;
            callback.args[0] = task;

            kaapi_stream_instruction_submit_kernel(device, task, callback);
            if (device->thread == ThreadWorker::self())
                device->offloader.launch_ready_instructions(KAAPI_STREAM_TYPE_KERN);
...
static inline void
kaapi_device_wait(kaapi_device_t * device)
{
    LOGGER_DEBUG("Waiting for device %d...", device->global_id);
    device->offloader.progress_pending_instructions(KAAPI_STREAM_TYPE_ALL, true);
}

kaapi_driver_t *
kaapi_driver_get(kaapi_driver_type_t type)
{
    kaapi_runtime_t * context = kaapi_runtime_get();
    return context->drivers.list + KAAPI_DRIVER_TYPE_CUDA;
}

kaapi_device_t *
kaapi_device_get(int device_global_id)
{
    kaapi_runtime_t * context = kaapi_runtime_get();
