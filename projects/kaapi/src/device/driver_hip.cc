...
typedef struct  kaapi_stream_hip_t
{
...
typedef struct  kaapi_device_hip_t
{
...
static kaapi_device_hip_t DEVICES[KAAPI_DEVICES_MAX];
...
static inline kaapi_device_hip_t *
__get_device_hip(int device_id)
{
    return (kaapi_device_hip_t *) __get_device(device_id);
}

/* Convert ptr driver device id (in [0..ngpus-1]) to the hip driver device id (in [0, INT_MAX[) */
static int CUDA_DEVICE_ID[KAAPI_DEVICES_MAX];

static inline void
__set_device_hip_id(int device_id, int hip_device_id)
{
    CUDA_DEVICE_ID[device_id] = hip_device_id;
    // LOGGER_DEBUG("driver device id = %d ; hip device id = %d", device_id, hip_device_id);
}
...
uint64_t hip_get_free_mem(int device_id)
{
    ...
    kaapi_device_hip_t * device = __get_device_hip(device_id);
...
static int hip_perf_topo[KAAPI_DEVICES_MAX][KAAPI_DEVICES_MAX];
...
__get_gpu_topo(void)
{
    ...
    for (int i = 0 ; i < hip_device_count ; ++i)
    {
        for (int j = 0 ; j < hip_device_count ; ++j)
        {
            if (hip_perf_topo[i][j] == -1)
            {
                ...
                LOGGER_INFO("Cuda GPU : no peer access from %d to %d", i, j);
            }
        }
    }
    ...
    for (int i = 0 ; i < hip_device_count ; ++i)
    {
        for (int j = 0 ; j < hip_device_count ; ++j)
        {
            ...
            LOGGER_INFO("Cuda GPU %d -> %d with perf = %d", i, j, rank);
        }
    }
}
...
KAAPI_DRIVER_ENTRYPOINT(init)(void)
{
    ...
    KAAPI_MUTEX_LOCK(DRIVER_MUTEX);
    {
        ...
        kaapi_runtime_t * ctx = kaapi_runtime_get();
        uint32_t ngpus = MIN(KAAPI_DRIVER_ENTRYPOINT(get_ndevices_max)(), ctx->conf.ngpus);
...
KAAPI_DRIVER_ENTRYPOINT(device_set_cpuset)(cpu_set_t * schedset, int device_id)
{
    ...
    assert(device_id >= 0);
    assert(device_id < KAAPI_DEVICES_MAX);
    ...
    if (err == 0)
    {
        ...
        LOGGER_WARN("Could not get a 'cpuset' for CUDA device %d, falling back to glibc...", device_id);
...
{
    assert(INITIALIZED);
    assert(device_id >= 0 && device_id < KAAPI_DEVICES_MAX);

    kaapi_device_hip_t * device = DEVICES + device_id;
...
static int
KAAPI_DRIVER_ENTRYPOINT(get_source)(int dst_global_id, int bitmask)
{
    ...
    int dst_driver_id = -1;
    for( int driver_dev_id = 0; driver_dev_id < KAAPI_DEVICES_MAX; driver_dev_id++ )
    {
         kaapi_device_hip_t* pdev = DEVICES + driver_dev_id;
         if( pdev->inherited.state == KAAPI_DEVICE_STATE_CREATE )
             continue;
    ...
    for( int driver_dev_id = 0; driver_dev_id < KAAPI_DEVICES_MAX; driver_dev_id++ )
    {
         ...
         if( !(pdev->inherited.state == KAAPI_DEVICE_STATE_COMMIT || pdev->inherited.state == KAAPI_DEVICE_STATE_RUNNING) )
...
# endif /* NDEBUG */
    ...
    for (int dev = 0 ; dev < KAAPI_DEVICES_MAX ; ++dev)
    {
        ...
        {
            kaapi_device_hip_t * odevice = __get_device_hip(j);
            if (odevice->inherited.state != KAAPI_DEVICE_STATE_INIT)
                continue ;
            ...
            if (access)
            {
                ...
                {
                    LOGGER_WARN("Could not enable peer from %d to %d", i, j);
                }
            }
            else
            {
                LOGGER_WARN("GPU peer from %d to %d is not possible", i, j);
            }
        }
    }
...
KAAPI_DRIVER_ENTRYPOINT(stream_instruction_launch)(
    ...
    kaapi_stream_hip_t * stream = (kaapi_stream_hip_t *) istream;
    assert(stream);

    switch (instr->type)
    {
        case (KAAPI_STREAM_INSTR_TYPE_NOP):
        {
            return 0;
        }
        ...
        case (KAAPI_STREAM_INSTR_TYPE_COPY_D2D):
        {
            ...
            LOGGER_INFO("hipMemcpy2DAsync(dst=%p, dpitch=%d, src=%p, spitch=%d, width=%d, height=%d, kind=%s",
...
KAAPI_DRIVER_ENTRYPOINT(stream_instructions_progress)(
    ...
    for (int p = istream->pending.pos.r ; p < istream->ok_p ; ++p)
    {
        ...
        switch (instr->type)
        {
            ...
            {
                /* unreachable code */
                LOGGER_FATAL("Unreachable code - instr->type=%d", instr->type);
                return EINVAL;
            }
        }
    }
...
KAAPI_DRIVER_ENTRYPOINT(stream_create)(
    ...
    uint8_t * mem = (uint8_t *) malloc(sizeof(kaapi_stream_hip_t) + capacity * sizeof(hipEvent_t));
    assert(mem);

    kaapi_stream_hip_t * stream = (kaapi_stream_hip_t *) mem;
    ...
    /* events */
    stream->hip.events.end = (hipEvent_t *) (mem + sizeof(kaapi_stream_hip_t));
