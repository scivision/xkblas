...
body_memory_coherent_async_fetch(void * vlauncher)
{
    ...
    static_assert(KAAPI_CALLBACK_ARGS_MAX >= 1);
    ...
    kaapi_driver_t * driver = kaapi_driver_get(KAAPI_DRIVER_TYPE_CUDA);
    assert(driver);

    kaapi_device_t * device = kaapi_device_get(fetch->src_device_global_id);
    assert(device);
...
static void
kaapi_memory_coherent_async_worker_thread_work(
    kaapi_runtime_t * runtime,
    ThreadWorker * thread,
    Task * current
) {
    assert(runtime);
    ...
    MemoryTree * memtree = runtime->get_memory_tree(args->ld, args->sizeof_type);
    assert(memtree);
...
static void
kaapi_memory_coherent_async_worker_thread_main_loop(kaapi_runtime_t * runtime)
{
    ...
    while (1)
    {
        ...
        kaapi_memory_coherent_async_worker_thread_work(runtime, thread, task);
    }
}

static void *
kaapi_memory_coherent_async_worker_thread_main(void * arg)
{
    kaapi_runtime_t * runtime = (kaapi_runtime_t *) arg;
    assert(runtime);
    ...
    kaapi_memory_coherent_async_worker_thread_main_loop(runtime);
...
void
kaapi_memory_coherent_async_worker_thread_init(kaapi_runtime_t * runtime)
{
    runtime->memory_coherent_worker_thread = NULL;

    pthread_t thread;
    int err = pthread_create(&thread, NULL, kaapi_memory_coherent_async_worker_thread_main, runtime);
    if (err)
...
# pragma message(TODO "'kaapi_memory_coherent_async' should take a row/col major parameter")

//  How 'kaapi_memory_coherent_async' works
...
extern "C"
void
kaapi_memory_coherent_async(
    int uplo, int memflag,
    int m, int n,
    void * kaapi, int ld,
    unsigned int sizeof_type
) {
    LOGGER_IMPL("in `kaapi_memory_coherent_async` - uplo and memflag parameters not supported");
    ...
    for (const auto & [conflicting_task, conflicting_accesses] : conflicts)
    {
        ...
        for (int access_id = 0 ; access_id < TASK_MAX_ACCESSES ; ++access_id)
        {
            if (conflicting_accesses[access_id])
            {
                ...
                #ifndef NDEBUG
                strncpy(task->label, "kaapi_memory_coherent_async", sizeof(task->label));
                #endif /* NDEBUG */
...
void
kaapi_memory_coherent_async_register_format(void)
{
    {
...

    {
        task_format_t format;
        memset(format.f, 0, sizeof(format.f));
        format.f[KAAPI_DRIVER_TYPE_CPU] = body_memory_coherent_async_fetch;
