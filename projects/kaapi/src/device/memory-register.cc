...
extern "C"
uint64_t
kaapi_register_memory_async(void * ptr, uint64_t size)
{
    # if USE_CUDA
...
extern "C"
int
kaapi_unregister_memory(void * ptr, uint64_t size)
{
    # if USE_CUDA
...
extern "C"
int
kaapi_unregister_memory_async(void * ptr, uint64_t size)
{
    return kaapi_unregister_memory( ptr, size ); // not async ...
}

extern "C"
int
kaapi_register_memory_waitall(void)
{
...
extern "C"
uint64_t
kaapi_register_memory(void * ptr, uint64_t size)
{
    uint64_t ret = kaapi_register_memory_async(ptr, size);
    kaapi_register_memory_waitall();
    return ret;
}
