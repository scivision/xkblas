...
static inline void
kaapi_device_memory_reset(kaapi_device_t * device)
{
    # pragma message(TODO "This is leaking")
    kaapi_alloc_chunk_t * chunk0 = (kaapi_alloc_chunk_t *) malloc(sizeof(kaapi_alloc_chunk_t));
    assert(chunk0);
    memcpy(chunk0, &(device->memdev.chunk0), sizeof(kaapi_alloc_chunk_t));
...
void
kaapi_device_memory_set_chunk0(
    ...
    device->memdev.chunk0.state         = KAAPI_ALLOC_CHUNK_STATE_FREE;
    ...
    kaapi_device_memory_reset(device);
}

static inline void
kaapi_device_init_memory(kaapi_device_t * device)
{
    KAAPI_MUTEX_INIT(device->memdev.lock);
    kaapi_device_memory_set_chunk0(device, 0, 0);
}

void
kaapi_memory_deallocate(
    kaapi_device_t * device,
    ...
    KAAPI_MUTEX_LOCK(device->memdev.lock);
    {
        chunk->state = KAAPI_ALLOC_CHUNK_STATE_FREE;
        chunk->use_counter = 0;

        /* can we merge chunk into next_chunk ? */
        kaapi_alloc_chunk_t * next_chunk = chunk->next;
        if (next_chunk && next_chunk->state == KAAPI_ALLOC_CHUNK_STATE_FREE)
        {
        ...
        if (prev_chunk)
        {
            ...
            if (prev_chunk->state == KAAPI_ALLOC_CHUNK_STATE_FREE)
            {
                if (delete_chunk)
                {
            ...
            else if (!delete_chunk)
            {
                ...
                while (prev_chunk && prev_chunk->state != KAAPI_ALLOC_CHUNK_STATE_FREE)
    ...
    KAAPI_MUTEX_UNLOCK(device->memdev.lock);
...
kaapi_memory_allocate(
    ...
    /* best fit strategy */
    kaapi_alloc_chunk_t * curr = device->memdev.free_chunk_list;
    kaapi_alloc_chunk_t * prevfree = NULL;
    size_t min_size = 0;
    kaapi_alloc_chunk_t * min_size_curr = NULL;
    kaapi_alloc_chunk_t * min_size_prevfree = NULL;
    ...
    {
        size_t curr_size = curr->size;
        kaapi_alloc_chunk_t * remainder = (kaapi_alloc_chunk_t *) malloc(sizeof(kaapi_alloc_chunk_t));
        remainder->device_ptr   = size + curr->device_ptr;
        remainder->size         = (curr_size - size);
        remainder->state        = KAAPI_ALLOC_CHUNK_STATE_FREE;
    ...
    if (curr != NULL)
    {
        ...
        curr->state = KAAPI_ALLOC_CHUNK_STATE_ALLOCATED;
        curr->freelink = NULL;
    }
...
void
kaapi_memory_deallocate_all(void)
{
    ...
    for (kaapi_device_global_id_t device_global_id = 0 ;
            ...
            ++device_global_id)
    {
        kaapi_device_t * device = kaapi_device_get(device_global_id);
        assert(device);
...
kaapi_device_commit(
    ...
    assert(device->state == KAAPI_DEVICE_STATE_INIT);
...
kaapi_device_init(
    ...
    kaapi_device_init_memory(device);
    ...
    assert(device->state == KAAPI_DEVICE_STATE_CREATE);
...
kaapi_device_create(
    ...
    if (drivers->devices.n == KAAPI_DEVICES_MAX)
        LOGGER_FATAL("Too many devices. Increase 'KAAPI_DEVICES_MAX' and recompile Xkblas");

    kaapi_driver_t * driver = drivers->list + driver_id;
    kaapi_device_t * device = driver->f_device_create(driver, device_driver_id);
    assert(device);
...
/////////////////////////
int
kaapi_device_poll(kaapi_device_t * device)
{
    int err = 0;
    assert(ThreadWorker::self() == device->thread);

    err = device->offloader.launch_ready_instructions(KAAPI_STREAM_TYPE_ALL);
    assert( (err == 0) || (err == EINPROGRESS));

    err = device->offloader.progress_pending_instructions(KAAPI_STREAM_TYPE_ALL, false);
    assert( (err == 0) || (err == EINPROGRESS));
...
static inline int
kaapi_device_accept_new_task(kaapi_device_t * device)
{
...
static inline void
kaapi_device_progress(
    kaapi_driver_t * driver,
    kaapi_device_t * device
) {
    int err = kaapi_device_poll(device);
    assert((err == 0) || (err == EINPROGRESS));
}

/* main loop for the thread responsible the passed device */
static inline int
kaapi_device_thread_main_loop(
    kaapi_driver_t * driver,
    kaapi_device_t * device
) {
    assert(ThreadWorker::self() == device->thread);
    assert(device->state == KAAPI_DEVICE_STATE_COMMIT);
    device->state = KAAPI_DEVICE_STATE_RUNNING;

    # pragma message(TODO "do we really need this mem_barrier here?")
    mem_barrier();

    ThreadWorker * worker = ThreadWorker::self();
    while (device->state == KAAPI_DEVICE_STATE_RUNNING)
    {
        Task * task;
        while ((task = worker->pop()) == NULL && device->offloader.is_empty(KAAPI_STREAM_TYPE_ALL))
            worker->pause();

        kaapi_device_progress(driver, device);
        if (task)
            kaapi_device_prepare_task(driver, device, task);
    }
...
void *
kaapi_device_thread_main(void * a)
{
    # pragma message(TODO "Implement device thread")

    kaapi_driver_device_thread_arg_t * arg = (kaapi_driver_device_thread_arg_t *) a;
    kaapi_drivers_t * drivers  = arg->drivers;
    ...
    /* init the device */
    kaapi_device_t * device = kaapi_device_create(drivers, driver_id, device_driver_id);
    kaapi_device_init(driver, device);
