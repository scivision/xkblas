...
void
kaapi_stats_task_state_incr(task_format_id_t fmtid, task_state_t state)
{
...
void
kaapi_stats_report(void)
{
    kaapi_runtime_t * context = kaapi_runtime_get();
    assert(context);
    kaapi_stats_t * stats = &(context->stats);
    ...
    for (int i = 0 ; i < KAAPI_STATS_TASK_FORMAT_MAX ; ++i)
    {
    ...
    LOGGER_WARN("streams:");
    for (int i = 0 ; i < KAAPI_STREAM_TYPE_ALL ; ++i)
    {
        LOGGER_WARN("  `%s`", kaapi_stream_type_to_str((kaapi_stream_type_t) i));
        LOGGER_WARN("    instructions");
        for (int j = 0 ; j < KAAPI_STREAM_INSTR_TYPE_MAX ; ++j)
        {
            if (stats->streams[i].instructions[j].launched.load())
            {
                LOGGER_WARN("      `%s`", kaapi_stream_instruction_type_to_str((kaapi_stream_instruction_type_t) j));
...
void
kaapi_stats_init(kaapi_stats_t * stats)
{
    memset(stats, 0, sizeof(kaapi_stats_t));
}

kaapi_stats_t *
kaapi_stats_get(void)
{
    return &(kaapi_runtime_get()->stats);
}
