...
class KMemoryForward {
    ...
    public:
        ...
        /* dst chunk */
        kaapi_alloc_chunk_t * chunk;

        /* the dst device */
        kaapi_device_global_id_t device_global_id;
...
class KMemoryBlock {
    ...
    public:
        ...
        MemoryReplicate replicates[KAAPI_DEVICES_MAX];

        /* valid devices (i.e. devices with at least one valid allocation) */
        volatile kaapi_device_global_id_bitfield_t valid;

        /* fetching devices (i.e. devices with at least one fetching allocation) */
        volatile kaapi_device_global_id_bitfield_t fetching;
    ...
    public:
        ...
        memory_block_init(
            ...
            for (kaapi_device_global_id_t device_global_id = 0 ; device_global_id < KAAPI_DEVICES_MAX ; ++device_global_id)
            {
...
class KMemoryTreeNodeSearch {
    ...
    public:
        class Partite {
            ...
            public:
                ...
                /* dst device */
                kaapi_device_global_id_t dst_device_global_id;
                ...
                kaapi_device_global_id_t src_allocation_view_id;

                /* src view */
                memory_replicate_view_t src_view;

                /* the source chunk for that partite */
                kaapi_alloc_chunk_t * src_chunk;
   ...
   public:
       ...
       const kaapi_device_global_id_t device_global_id;
        ...
        */
        ...
        ///////////////////////////////////
        size_t bytes_owned[KAAPI_DEVICES_MAX];

   public:
       KMemoryTreeNodeSearch() : KMemoryTreeNodeSearch(0) {}

       KMemoryTreeNodeSearch(
           kaapi_device_global_id_t devid
       ) :
       ...
       void
       prepare_search_awaiting(kaapi_alloc_chunk_t * chunk)
       {
           this->chunk = chunk;
    ...
    public:
        ...
        dump_cube_str(FILE * f) const
        {
         ...
         // for (uint8_t device_global_id = 0 ; device_global_id < ctx->drivers.devices.n ; ++device_global_id)
            for (kaapi_device_global_id_t device_global_id = 0 ; device_global_id < KAAPI_DEVICES_MAX+1 ; ++device_global_id)
            {
...
class KMemoryTree : public KHPTree<K, KMemoryTreeNodeSearch<K>, CUT>, Lockable {
    ...
    public:
        ...
        typedef struct  fetch_t
        {
            ...
            /* src device id */
            kaapi_device_global_id_t src_device_global_id;

            /* src view */
            memory_replicate_view_t src_view;

            /* mark 'fetched' all the tasks awaiting on that allocation */
            kaapi_alloc_chunk_t * dst_chunk;
        ...
        {
            /* a fetch completed */
            LOGGER_DEBUG("Task `%s` fetched `%p`", task->label, fetch->dst_chunk->device_ptr);
            if (task->fetched() == TASK_STATE_DATA_FETCHED)
            {
                kaapi_device_t * device = kaapi_device_get(fetch->dst_device_global_id);
                kaapi_device_task_execute(device, task);
                # pragma message(TODO "Here, we are not polling the offloader kernel streams... Do we want to ?")
            }
        }

        static void
        fetch_callback(const void * args[KAAPI_CALLBACK_ARGS_MAX])
        {
            assert(KAAPI_CALLBACK_ARGS_MAX >= 3);
            ...
            LOGGER_DEBUG("Fetch completed for allocation `%p`", fetch->dst_chunk->device_ptr);
            ...
            if (nforwards)
            {
                ...
                for (size_t i = 0 ; i < nforwards ; ++i)
                {
                    ...
                    assert(0 <= forward.device_global_id && forward.device_global_id < KAAPI_DEVICES_MAX);

                    kaapi_device_t * device = kaapi_device_get(forward.device_global_id);
                    assert(device);
        ...
        fetch_access_find_src(
            ...
            kaapi_device_global_id_t dst_device_global_id,
            kaapi_device_global_id_bitfield_t valid
        ) {
            kaapi_device_global_id_bitfield_t src = driver->f_get_source(dst_device_global_id, valid);
            if (src == -1) // Driver failed to find a valid source
                src = (kaapi_device_global_id_bitfield_t) (__builtin_ffs(valid) - 1);
            assert(src >= 0);
            return src;
        }
        ...
        fetch_list_to_host_setup_partition(Partition & partition)
        {
            ...
            for (Partite & partite : partition.partites)
            {
                ...
                // kaapi_device_global_id_t src = __random_set_bit(partite.block->valid) - 1;
                kaapi_device_global_id_t src = (kaapi_device_global_id_t) (__builtin_ffs(partite.block->valid) - 1);
                assert(src >= 0);
            ...
            auto f = [device, size, &freed](NodeBase * nodebase, void * args, bool & stop) {
                ...
                if (!valid_on_any_device || valid_on_any_other_devices)
                {
                    ...
                    for (int i = 0 ; i < replicate.nallocations ; ++i)
                    {
                        ...
                        {
                            LOGGER_WARN("Evicted a block of size %u MB", allocation->chunk->size/1024/1024);
                            kaapi_memory_deallocate(device, allocation->chunk);
        ...
        fetch_access_allocate(
            ...
            kaapi_alloc_chunk_t * chunk = nullptr;
            int retry_cnt = 0;

            do {

                chunk = kaapi_memory_allocate(driver, device, size);
                if (chunk)
                    return chunk;

                // TODO : polling is risky here, because it may take a lock on the
                // memory tree, and 'kaapi_memory_allocate' is called within a
                // memory-tree lock => double-lock deadlock

                // kaapi_device_poll(device);
...
next_view:
        ...
        fetch_access_find_allocation(
            ...
            kaapi_alloc_chunk_t * chunk = this->fetch_access_find_allocation_continuous(device, partition);
            if (chunk == nullptr)
            {
        ...
        fetch_access_setup_copies(
            ...
            {
                const kaapi_device_global_id_bitfield_t dst_devbit = (1 << device->global_id);
                ...
                for (Partite & partite : partition.partites)
                {
                    ...
                    LOGGER_DEBUG("Task `%s` fetching one by `%s` on `%p`", task->label, (dst_replicate.fetching & dst_allocbit) ? "awaiting" : "launching", dst_allocation_view->view.addr);
                    ...
                    if (partite.block->valid)
                    {
                        ...
                        /* get a valid source */
                        kaapi_device_global_id_t src = driver->f_get_source(device->global_id, partite.block->valid);
                    ...
                    else if (partite.block->fetching)
                    {
                        ...
                        kaapi_device_global_id_t fetching = driver->f_get_source(device->global_id, partite.block->fetching);
                        assert(0 <= fetching && fetching < KAAPI_DEVICES_MAX);
        ...
        fetch_access_set_valid(
            ...
            if (access->mode & ACCESS_MODE_W)
            {
                ...
                for (Partite & partite : partition.partites)
                {
                    ...
                    for (kaapi_device_global_id_t device_global_id = 0 ;
                            device_global_id < KAAPI_DEVICES_MAX ;
                            ++device_global_id)
                    {
        ...
        on_intersect(
            ...
            switch (search.type)
            {
                ...
                {
                    const kaapi_device_global_id_bitfield_t devbit = (kaapi_device_global_id_bitfield_t) (1 << search.device_global_id);
