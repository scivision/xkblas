...
# include <kaapi/device/memory-access.hpp>
...
# if USE_STATS
void kaapi_stats_task_state_incr(task_format_id_t fmtid, task_state_t state);
# else
#  define kaapi_stats_task_state_incr(...)
# endif /* USE_STATS */
...
class alignas(CACHE_LINE_SIZE) KTask
{
    ...
    public:
        ...
        /* worker id on where to schedule once ready (or KAAPI_DEVICES_MAX if
    ...
    public:
            ...
            state({.lock=0, .value=TASK_STATE_ALLOCATED})
        {
            ...
            # endif /* NDEBUG */
            kaapi_stats_task_state_incr(this->fmtid, TASK_STATE_ALLOCATED);
        }

        virtual ~KTask()
        {
            this->state.value = TASK_STATE_DEALLOCATED;
            kaapi_stats_task_state_incr(this->fmtid, TASK_STATE_DEALLOCATED);
        }
    ...
    public:
        ...
        commit(void)
        {
            ...
            if (this->wc.fetch_sub(1, std::memory_order_seq_cst) - 1 == 0)
            {
                ...
                kaapi_stats_task_state_incr(this->fmtid, TASK_STATE_READY);
                kaapi_task_ready(this);
            }
        }
        ...
        fetching(const uint16_t n = 1)
        {
            if (this->wc.fetch_add(n, std::memory_order_seq_cst) == 0)
            {
                ...
                kaapi_stats_task_state_incr(this->fmtid, TASK_STATE_DATA_FETCHING);
            }
        }
        ...
        fetched(const uint16_t n = 1)
        {
            ...
            if (this->wc.fetch_sub(n, std::memory_order_seq_cst) == 1)
            {
                ...
                kaapi_stats_task_state_incr(this->fmtid, TASK_STATE_DATA_FETCHED);
        ...
        complete(void)
        {
            ...
            SPINLOCK_LOCK(this->state.lock);
            {
                ...
                kaapi_stats_task_state_incr(this->fmtid, TASK_STATE_COMPLETED);
            }
            SPINLOCK_UNLOCK(this->state.lock);
...
void kaapi_task_ready(Task * task);
