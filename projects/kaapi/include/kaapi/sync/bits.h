...
static inline kaapi_device_global_id_t
__random_set_bit(kaapi_device_global_id_bitfield_t bitfield)
{
    ...
    static_assert(sizeof(kaapi_device_global_id_bitfield_t) <= sizeof(int));

    const int nb = __builtin_popcount(bitfield);
    kaapi_device_global_id_t idx = 0;
    int k = rand_r(&seed) % nb;
    for (int i = 0; i <= k; ++i)
    {
        idx = static_cast<kaapi_device_global_id_t>(__builtin_ffs(static_cast<int>(bitfield)));
        bitfield &= ~(1 << (idx - 1));
    }
