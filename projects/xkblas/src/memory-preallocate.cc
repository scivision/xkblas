...
/*   Updated: 2025/06/25 17:02:06 by Romain PEREIRA         / _______ \       */
/*                                                          \_)     (_/       */
/*   License: CeCILL-C                                                        */
/*                                                                            */
...
xkblas_memory_preallocate(
    ...
    for (xkrt_device_global_id_t device_global_id = 0; device_global_id < runtime->drivers.devices.n ; ++device_global_id)
        if (device_global_id != HOST_DEVICE_GLOBAL_ID)
            xkrt_coherent_allocate_2D(runtime, device_global_id, MATRIX_COLMAJOR, ptr, (size_t) ld, (size_t) m, (size_t) n, sizeof_type);
}
