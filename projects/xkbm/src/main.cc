...
/*   Updated: 2025/06/03 18:39:43 by Romain PEREIRA         / _______ \       */
/*                                                          \_)     (_/       */
/*   License: CeCILL-C                                                        */
/*                                                                            */
...
void   aml_benchmark_push(benchmark_node_t *);
...
main(void)
{
    ...
    for (unsigned int i = 0 ; i < sizeof(backends) / sizeof(backend_t) ; ++i)
    {
        ...
        LOGGER_INFO("%9s | %9s", backend->name, yn[enabled]);
    }
    LOGGER_INFO("----------------------------------");
    ...
    for (unsigned int i = 0 ; i < sizeof(backends) / sizeof(backend_t) ; ++i)
    {
        ...
        if (backend->enabled && backend->init)
            backend->init();
    }
