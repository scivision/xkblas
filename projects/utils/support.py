...
if len(sys.argv) != 12:
    print("usage: {} [DST-DIR] [DST-FILE] [USE_CUDA] [USE_ZE] [USE_SYCL] [USE_CL] [USE_STATS] [USE_CAIRO] [USE_NVML] [USE_ZES] [USE_RSMI]".format(sys.argv[0]))
    sys.exit(0)
...
use_nvml=cmake_option_convert(sys.argv[9])
use_zes=cmake_option_convert(sys.argv[10])
use_rsmi=cmake_option_convert(sys.argv[11])
cmd('sed -e "s/__XKRT_SUPPORT_CUDA_VALUE__/{}/g" -e "s/__XKRT_SUPPORT_ZE_VALUE__/{}/g" -e "s/__XKRT_SUPPORT_SYCL_VALUE__/{}/g" -e "s/__XKRT_SUPPORT_CL_VALUE__/{}/g" -e "s/__XKRT_SUPPORT_STATS_VALUE__/{}/g" -e "s/__XKRT_SUPPORT_CAIRO_VALUE__/{}/g" -e "s/__XKRT_SUPPORT_NVML_VALUE__/{}/g" -e "s/__XKRT_SUPPORT_ZES_VALUE__/{}/g" -e "s/__XKRT_SUPPORT_RSMI_VALUE__/{}/g" {}/{}.template > {}/{}-tmp.h'.format(use_cuda, use_ze, use_sycl, use_opencl, use_stats, use_cairo, use_nvml, use_zes, use_rsmi, dstdir, dstfile, dstdir, dstfile))
