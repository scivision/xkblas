#!/bin/python

import os
import sys

if len(sys.argv) != 6:
    print("usage: {} [DST-DIR] [DST-FILE] [USE_HOST] [USE_CUDA] [USE_ZE]".format(sys.argv[0]))
    sys.exit(0)

dstdir=sys.argv[1]
dstfile=sys.argv[2]

def cmd(s):
    print(s)
    return os.system(s)

def cmake_option_convert(opt):
    opt = opt.lower()
    if opt == "off" or opt == "false" or opt == "0":
        return 0
    return 1

use_host=cmake_option_convert(sys.argv[3])
use_cuda=cmake_option_convert(sys.argv[4])
use_ze=cmake_option_convert(sys.argv[5])
cmd('sed -e "s/__XKRT_SUPPORT_HOST_VALUE__/{}/g" -e "s/__XKRT_SUPPORT_CUDA_VALUE__/{}/g" -e "s/__XKRT_SUPPORT_ZE_VALUE__/{}/g" {}/{}.template > {}/{}-tmp.h'.format(use_host, use_cuda, use_ze, dstdir, dstfile, dstdir, dstfile))


if cmd("test -f {}/{}.h".format(dstdir, dstfile)) or cmd("diff {}/{}.h {}/{}-tmp.h".format(dstdir, dstfile, dstdir, dstfile)):
    cmd("cp {}/{}-tmp.h {}/{}.h".format(dstdir, dstfile, dstdir, dstfile))

cmd("rm {}/{}-tmp.h".format(dstdir, dstfile))
