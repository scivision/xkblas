...
XKRT_DRIVER_ENTRYPOINT(device_set_cpuset)(cpu_set_t * schedset, int device_driver_id)
{
    ...
    HWLOC_SAFE_CALL(hwloc_levelzero_get_device_cpuset(XKRT_HWLOC_TOPOLOGY, device->ze_device, cpuset));
    CPU_ZERO(schedset);
...
XKRT_DRIVER_ENTRYPOINT(device_init)(int device_driver_id)
{
    ...
    assert(context);
    size_t free = device->ze_properties.maxMemAllocSize; // TODO : can we get the available free memory instead ?;

    const size_t size = (const size_t) ((double)free * (double)(context->conf.gpu_mem_percent / 100.0));
    ...
    xkrt_device_memory_set_chunk0(&(device->inherited), (uintptr_t) device_ptr, reserve_size);
}
