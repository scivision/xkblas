...
body_memory_coherent_async_fetch(void * vlauncher)
{
    ...
    xkrt_device_t * device = xkrt_device_get(fetch->src_device_global_id);
    assert(device);
...
xkrt_memory_coherent_async(
    ...
    LOGGER_DEBUG("`xkrt_memory_coherent_async` found %d conflicts", conflicts.size());
...
void
xkrt_memory_coherent_async_register_format(void)
{
    {
...

    {
        task_format_t format;
        memset(format.f, 0, sizeof(format.f));
        format.f[XKRT_DRIVER_TYPE_CPU] = body_memory_coherent_async_fetch;
