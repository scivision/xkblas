...
XKRT_DRIVER_ENTRYPOINT(init)(void)
{
    ...
    XKRT_MUTEX_LOCK(DRIVER_MUTEX);
    {
        ...
        uint32_t ngpus = MIN(XKRT_DRIVER_ENTRYPOINT(get_ndevices_max)(), context->conf.ngpus);
...
XKRT_DRIVER_ENTRYPOINT(stream_instructions_progress)(
    ...
    for (xkrt_stream_instruction_counter_t p = istream->pending.pos.r ; p < istream->ok_p ; ++p)
        istream->complete(p);
