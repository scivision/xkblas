...
{
    return (xkrt_device_hip_t *) __get_device(device_id);
}

/* Convert ptr driver device id (in [0..ngpus-1]) to the hip driver device id (in [0, INT_MAX[) */
static int CUDA_DEVICE_ID[XKRT_DEVICES_MAX];
...
uint64_t hip_get_free_mem(int device_id)
{
    ...
    xkrt_device_hip_t * device = __get_device_hip(device_id);
...
static int hip_perf_topo[XKRT_DEVICES_MAX][XKRT_DEVICES_MAX];
...
XKRT_DRIVER_ENTRYPOINT(init)(void)
{
    ...
    XKRT_MUTEX_LOCK(DRIVER_MUTEX);
    {
        ...
        uint32_t ngpus = MIN(XKRT_DRIVER_ENTRYPOINT(get_ndevices_max)(), ctx->conf.ngpus);
    ...
    # if 0
    int nrrl = getenv("XKRT_RRL_SIZE") ? atoi(getenv("XKRT_RRL_SIZE")) : 1;
    if (nrrl <= 0)
        nrrl = 1;
    ...
    {
        xkrt_hip_init_reqreg_list(&all_rrl[i]);
        int err = pthread_create(&all_rrl[i].thread, 0, xkrt_hip_register_thread, (void*)(uintptr_t)i );
        assert(err ==0);
    }
    # endif
...
XKRT_DRIVER_ENTRYPOINT(device_set_cpuset)(cpu_set_t * schedset, int device_id)
{
    ...
    assert(device_id >= 0);
    assert(device_id < XKRT_DEVICES_MAX);
...
{
    assert(INITIALIZED);
    assert(device_id >= 0 && device_id < XKRT_DEVICES_MAX);

    xkrt_device_hip_t * device = DEVICES + device_id;
...
XKRT_DRIVER_ENTRYPOINT(device_init)(int device_id)
{
    ...
    __check_error(res);
    xkrt_device_memory_set_chunk0(&(device->inherited), device_ptr, size);
}
...
static int
XKRT_DRIVER_ENTRYPOINT(get_source)(int dst_global_id, int bitmask)
{
    ...
    int dst_driver_id = -1;
    for( int driver_dev_id = 0; driver_dev_id < XKRT_DEVICES_MAX; driver_dev_id++ )
    {
         xkrt_device_hip_t* pdev = DEVICES + driver_dev_id;
         if( pdev->inherited.state == XKRT_DEVICE_STATE_CREATE )
             continue;
    ...
    for( int driver_dev_id = 0; driver_dev_id < XKRT_DEVICES_MAX; driver_dev_id++ )
    {
         ...
         if( !(pdev->inherited.state == XKRT_DEVICE_STATE_COMMIT || pdev->inherited.state == XKRT_DEVICE_STATE_RUNNING) )
...
# endif /* NDEBUG */
    ...
    for (int dev = 0 ; dev < XKRT_DEVICES_MAX ; ++dev)
    {
        ...
        {
            # if 0
            device->affinity[0] |= (1UL<<xkrt_memory_asid_get_lid(xkrt_device_list[j]->inherited.memdev.asid));
            ld->affinity[0] |= (1UL<<xkrt_memory_asid_get_lid(xkrt_device_list[j]->inherited.memdev.asid));
            # endif
        }
        else
        {
            xkrt_device_hip_t * odevice = __get_device_hip(j);
            if (odevice->inherited.state != XKRT_DEVICE_STATE_INIT)
                continue ;
            ...
            if (access)
            {
                ...
                if ((res == hipSuccess) || (res == hipErrorPeerAccessAlreadyEnabled))
                {
                    ...
                    {
                        # if 0
                        device->affinity[rank-1] |= (1UL <<xkrt_memory_asid_get_lid(xkrt_device_list[j]->inherited.memdev.asid));
                        # endif
                    }
                }
                else
...
XKRT_DRIVER_ENTRYPOINT(stream_instruction_launch)(
    ...
    switch (instr->type)
    {
        case (XKRT_STREAM_INSTR_TYPE_NOP):
        {
            return 0;
        }
...
XKRT_DRIVER_ENTRYPOINT(stream_create)(
    ...
    /* events */
    stream->hip.events.end = (hipEvent_t *) (mem + sizeof(xkrt_stream_hip_t));
