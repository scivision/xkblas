...
*/
typedef struct  xkrt_alloc_chunk_t
{
    uintptr_t device_ptr;                   /* position of memory in device */
    size_t size;                            /* size of the segment in byte */
    int state;                              /* state of the chunk */
    struct xkrt_alloc_chunk_t * prev;     /* previous chunk in double chained list */
    struct xkrt_alloc_chunk_t * next;     /* next chunk in double chained list */
    struct xkrt_alloc_chunk_t * freelink; /* next freechunk in the chained list */
...
typedef struct  xkrt_device_memory_t
{
    xkrt_mutex_t lock;
    xkrt_alloc_chunk_t chunk0;
    xkrt_alloc_chunk_t * free_chunk_list;

}               xkrt_device_memory_t;
