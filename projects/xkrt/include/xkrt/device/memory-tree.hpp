...
class KMemoryTreeNodeSearch {
    ...
    public:
        class Partite {
            ...
            public:
                ...
                xkrt_device_global_id_t src_allocation_view_id;

                /* src view */
                memory_replicate_view_t src_view;

                /* the source chunk for that partite */
                xkrt_alloc_chunk_t * src_chunk;
   ...
   public:
       ...
       void
       prepare_search_awaiting(xkrt_alloc_chunk_t * chunk)
       {
           this->chunk = chunk;
...
class KMemoryTree : public KHPTree<K, KMemoryTreeNodeSearch<K>, CUT>, Lockable {
    ...
    public:
        ...
        fetch_callback_task(fetch_t * fetch, Task * task)
        {
            ...
            {
                xkrt_device_t * device = xkrt_device_get(fetch->dst_device_global_id);
        ...
        fetch_callback(const void * args[XKRT_CALLBACK_ARGS_MAX])
        {
            ...
            if (nforwards)
            {
                ...
                for (size_t i = 0 ; i < nforwards ; ++i)
                {
                    ...
                    assert(0 <= forward.device_global_id && forward.device_global_id < XKRT_DEVICES_MAX);

                    xkrt_device_t * device = xkrt_device_get(forward.device_global_id);
                    assert(device);
        ...
        fetch_access_find_src(
            ...
            xkrt_device_global_id_t dst_device_global_id,
            xkrt_device_global_id_bitfield_t valid
        ) {
            xkrt_device_global_id_bitfield_t src = driver->f_get_source(dst_device_global_id, valid);
            if (src == -1) // Driver failed to find a valid source
                src = (xkrt_device_global_id_bitfield_t) (__builtin_ffs(valid) - 1);
            assert(src >= 0);
            return src;
        }
                ...
                if (!valid_on_any_device || valid_on_any_other_devices)
                {
                    ...
                    for (int i = 0 ; i < replicate.nallocations ; ++i)
                    {
                        ...
                        if (allocation->chunk->use_counter == 1)
                        {
                            ...
                            xkrt_memory_deallocate(device, allocation->chunk);
        ...
        fetch_access_allocate(
            ...
            xkrt_alloc_chunk_t * chunk = nullptr;
            int retry_cnt = 0;

            do {

                chunk = xkrt_memory_allocate(driver, device, size);
                if (chunk)
                    return chunk;
...
next_view:
        ...
        fetch_access_find_allocation(
            ...
            xkrt_alloc_chunk_t * chunk = this->fetch_access_find_allocation_continuous(device, partition);
            if (chunk == nullptr)
            {
        ...
        fetch_access_setup_copies(
            ...
            {
                const xkrt_device_global_id_bitfield_t dst_devbit = (1 << device->global_id);
                ...
                for (Partite & partite : partition.partites)
                {
                    ...
                    if (partite.block->valid)
                    {
                        ...
                        /* get a valid source */
                        xkrt_device_global_id_t src = driver->f_get_source(device->global_id, partite.block->valid);
                    ...
                    else if (partite.block->fetching)
                    {
                        ...
                        xkrt_device_global_id_t fetching = driver->f_get_source(device->global_id, partite.block->fetching);
