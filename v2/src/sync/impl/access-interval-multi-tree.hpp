...
class AccessIntervalMultiTree : public History<K, T> {
    ...
    class Node {
        ...
        public:
            ...
            Node(Region<K> & r) : Node(r, 0, BLACK) {}

            Node(Region<K> & r, int k, Color color) :
                parent(nullptr),
                k(k),
                region(r),
                colors{BLACK},
            ...
            inherit_accesses(Node * parent)
            {
                this->ins.insert(
                    this->ins.end(),
                    std::make_move_iterator(parent->ins.begin()),
            ...
            dump(FILE * f) const
            {

                {
                    ...
                    char intervals[1024];
                    this->region.tostring(intervals, sizeof(intervals));

                    char include_intervals[1024];
                    this->includes.region.tostring(include_intervals, sizeof(include_intervals));

                    fprintf(f, "    N%p[fontcolor=\"#ffffff\", label=\"--- node ---\\nk=%d\\n%s\\nins=%s\\nout=%s\\n\\n--- includes ---\\n%s\\nnout=%d\\nsize=%d\\nnelements={%d, %d}\\nheight=%d\", style=filled, fillcolor=\"%s\"] ;\n", this, this->k, intervals, ins, out, include_intervals, this->includes.nout, this->size(), this->includes.nelements[0], this->includes.nelements[1], this->height(), color);
                }
            ...
            void
            dump_regions(FILE * f) const
            {
                assert(K == 1 || K == 2);
                if (K == 1)
                {
                    fprintf(f, "    \\draw (%d,-%d) rectangle (%d,-%d) node[midway] {[%d..%d[ \\\\ ins: %ld \\\\ out:     $%d$};\n",
                        this->region[0].a, 0,
                        this->region[0].b, 2,
                ...
                {
                    fprintf(f, "    \\draw (%d,-%d) rectangle (%d,-%d) node[midway] {[%d..%d[ x [%d..%d[ \\\\ ins: %ld \\\\ out: %d};\n",
    ...
    public:
        ...
        export_pdf(void) const
        {
            ...
            file = fopen("regions.tex", "w");
            this->dump_regions(file);
            fclose(file);
            r = system("pdflatex -interaction=nonstopmode regions.tex > /dev/null 2>&1");
            if (r)
...
# ifdef CUT
        ...
        insert_from_cut(
            ...
            parent->region.set_intervals(region.intervals);
            parent->register_access(mode, obj);

            parent->includes.region.set_intervals(region.intervals);
            parent->includes.nout = 1;
...
# endif /* CUT */
        ...
        insert_from(
            ...
            DEBUG("---  (start) subinsert of dimension %d and type %s for obj %p", K, (mode == IN) ? "in" : (mode == OUT) ? "out" : "unkn", obj);
            for (int i = 0 ; i < K ; ++i)
            ...
            while (k < K)
            {
                // quick-way out, if the region includes all subregions with an
...
# endif /* CUT */
            ...
            DEBUG("---  (done) subinsert of dimension %d and type %s for obj %p", K, (mode == IN) ? "in" : (mode == OUT) ? "out" : "unkn", obj);
        }
...
# endif /* NDEBUG */
        ...
        intersect_from(Node * node, access_mode_t mode, Region<K> & region, T & obj) const
        {
            ...
            if (mode == IN && node->includes.nout == 0)
                return ;
        ...
        // Dump represented regions to the given file
        void
        dump_regions(FILE * f) const
        {
    ...
    public:
        ...
        {
            (void) args;
            auto f = std::bind(&AccessIntervalMultiTree<K>::coherency_intervals_includes_check, this, _1, _2);
        ...
        {
            auto f = std::bind(&AccessIntervalMultiTree<K>::coherency_intervals_includes_foreach, this, _1, _2);
        ...
        {
            Node * root = (Node *) args;
            auto f = std::bind(&AccessIntervalMultiTree<K>::coherency_intervals_disjoint_compare, this, _1, _2);
        ...
        {
            auto f = std::bind(&AccessIntervalMultiTree<K>::coherency_intervals_disjoint_for, this, _1, _2);
            foreach_node(root, f, root);
        }
