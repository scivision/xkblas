...
xkblas_cgemm_tile_async(
    ...
    const float _Complex *A, int Atm, int Atn, int LDA,
    const float _Complex *B, int Btm, int Btn, int LDB,
    const float _Complex* beta,
    float _Complex *C, int Ctm, int Ctn, int LDC
) {
            ...
            if (transA == CblasNoTrans)
            {
                if (transB == CblasNoTrans)
                {
                    for (int tk = 0; tk < Ant; ++tk)
                    {
                        ...
                        float _Complex zbeta = (tk == 0) ? *beta : 1.0;
                        xkblas_cgemm_tile_async(
                                transA, transB,
                                bs_mm, bs_nn, bs_kn,
                                alpha,
