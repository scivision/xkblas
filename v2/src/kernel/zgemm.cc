...
xkblas_zgemm_tile_async(
    ...
    const double _Complex *A, int Atm, int Atn, int LDA,
    const double _Complex *B, int Btm, int Btn, int LDB,
    const double _Complex* beta,
    double _Complex *C, int Ctm, int Ctn, int LDC
) {
            ...
            if (transA == CblasNoTrans)
            {
                if (transB == CblasNoTrans)
                {
                    for (int tk = 0; tk < Ant; ++tk)
                    {
                        ...
                        double _Complex zbeta = (tk == 0) ? *beta : 1.0;
                        xkblas_zgemm_tile_async(
                                transA, transB,
                                bs_mm, bs_nn, bs_kn,
                                alpha,
