...
int testing_ztrmm(int argc, char **argv)
{
    ...
    printf("------ TESTS FOR XKBLAS WRAPPER API ZTRMM ROUTINE -------  \n");
#else
    printf("------ TESTS FOR XKBLAS ZTRMM ROUTINE -------  \n");
#endif
                          ...
                          Complex64_t *Bref, Complex64_t *Bcham, int LDB)
{
    ...
    Anorm       = LAPACKE_zlantr_work(LAPACK_COL_MAJOR, 'I', uplo, diag,
                                Am, An, A, LDA, work);
    ...
    xkblas_ztrmm_native(side, uplo, trans, diag, M, N, &alpha, A, LDA, Bref, LDB);
