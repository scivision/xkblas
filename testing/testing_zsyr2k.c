...
int testing_zsyr2k(int argc, char **argv)
{
    ...
    printf("------ TESTS FOR XKBLAS WRAPPER API ZSYR2K ROUTINE -------  \n");
#else
    printf("------ TESTS FOR XKBLAS ZSYR2K ROUTINE -------  \n");
#endif
...
#define ITER 5
            ...
            for (k=0; k<ITER; ++k)
            {
              ...
              zsyr2k_(&up, &tr, &N, &K, &alpha, A, &LDA, B, &LDB, &beta, Cfinal, &LDC);
                          ...
                          Complex64_t beta, Complex64_t *Cref, Complex64_t *Ccham, int LDC)
{
    ...
    xkblas_zsyr2k_native(uplo, trans, N, K, &alpha, A, LDA, B, LDB, &beta, Cref, LDC);
