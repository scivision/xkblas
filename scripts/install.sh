...
XKBLAS_BRANCH=v2.0-sync-versions

XKAAPI_CMAKE_BUILD_TYPE="Debug"
XKBLAS_CMAKE_BUILD_TYPE="Debug"
...
buildir=$REPO_DIRECTORY/xkaapi/build/$XKAAPI_HASH/$XKAAPI_CMAKE_BUILD_TYPE
XKAAPI_INSTALL_DIR=$INSTALL_DIRECTORY/xkaapi/$XKAAPI_HASH/$XKAAPI_CMAKE_BUILD_TYPE
mkdir -p $buildir
cd $buildir
CMAKE_PREFIX_PATH=$CUDA_PATH:$CMAKE_PREFIX_PATH cmake $CMAKE_XKAAPI_OPTS -DCMAKE_BUILD_TYPE="$XKAAPI_CMAKE_BUILD_TYPE" -DCMAKE_INSTALL_PREFIX="$XKAAPI_INSTALL_DIR" $REPO_DIRECTORY/xkaapi
make install -j
...
buildir=$REPO_DIRECTORY/xkblas/build/$XKBLAS_HASH/$XKBLAS_CMAKE_BUILD_TYPE
XKBLAS_INSTALL_DIR=$INSTALL_DIRECTORY/xkblas/$XKBLAS_HASH/$XKBLAS_CMAKE_BUILD_TYPE
mkdir -p $buildir
cd $buildir
CMAKE_PREFIX_PATH=$CUDA_PATH:$XKAAPI_INSTALL_DIR:$CMAKE_PREFIX_PATH cmake $CMAKE_XKBLAS_OPTS -DCMAKE_BUILD_TYPE="$XKBLAS_CMAKE_BUILD_TYPE" -DCMAKE_INSTALL_PREFIX="$XKBLAS_INSTALL_DIR" $REPO_DIRECTORY/xkblas
make install -j
...
XKAAPI_MODULE_FILE=$XKAAPI_MODULE_DIRECTORY/$XKAAPI_CMAKE_BUILD_TYPE
XKBLAS_MODULE_FILE=$XKBLAS_MODULE_DIRECTORY/$XKBLAS_CMAKE_BUILD_TYPE
...
echo "Success. You may type
    ...
    module load xkaapi/$XKAAPI_HASH/$XKAAPI_CMAKE_BUILD_TYPE
    module load xkblas/$XKBLAS_HASH/$XKBLAS_CMAKE_BUILD_TYPE
