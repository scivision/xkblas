/* ************************************************************************** */
/*                                                                            */
/*   syrk.cc                                                      .-*-.       */
/*                                                              .'* *.'       */
/*   Created: 2024/10/03 15:23:28 by Romain Pereira          __/_*_*(_        */
/*   Updated: 2026/01/08 16:32:51 by Romain PEREIRA         / _______ \       */
/*                                                          \_)     (_/       */
/*   License: CeCILL-C                                                        */
/*                                                                            */
/*   Author: Pierre-Etienne POLET <pierre-etienne.polet@inria.fr>             */
/*   Author: Thierry GAUTIER <thierry.gautier@inrialpes.fr>                   */
/*   Author: Romain PEREIRA <romain.pereira@inria.fr>                         */
/*                                                                            */
/*   Copyright: see AUTHORS                                                   */
/*                                                                            */
/* ************************************************************************** */

# include <xkblas/xkblas.hpp>

XKRT_NAMESPACE_USE;

extern "C"
int
xkblas_£syrk_tile_async(
    int uplo, int trans,
    const int n, const int k,
    const TYPE * alpha,
    const TYPE * A, const int Atm, const int Atn, const int Amb, const int Anb, const int lda,
    const TYPE * beta,
          TYPE * C, const int Ctm, const int Ctn, const int Cmb, const int Cnb, const int ldc,
    xkrt_device_unique_id_t device_unique_id
) {
    return xkblas_get()->syrk_tile_async<xkblas_precision_t::££>(uplo, trans, n, k, alpha, A, Atm, Atn, Amb, Anb, lda, beta, C, Ctm, Ctn, Cmb, Cnb, ldc, device_unique_id);
}

extern "C"
int
xkblas_£syrk_async(
    int uplo, int trans,
    int n, int k,
    const TYPE * alpha,
    const TYPE * A, int lda,
    const TYPE * beta,
          TYPE * C, int ldc
) {
    return xkblas_get()->syrk_async<xkblas_precision_t::££>(uplo, trans, n, k, alpha, A, lda, beta, C, ldc);
}

extern "C"
int
xkblas_£syrk_sync(
    int uplo, int trans,
    int n, int k,
    const TYPE * alpha,
    const TYPE * A, int lda,
    const TYPE * beta,
          TYPE * C, int ldc
) {
    return xkblas_get()->syrk_sync<xkblas_precision_t::££>(uplo, trans, n, k, alpha, A, lda, beta, C, ldc);
}

extern "C"
int
xkblas_£syrk(
    int uplo, int trans,
    int n, int k,
    const TYPE * alpha,
    const TYPE * A, int lda,
    const TYPE * beta,
          TYPE * C, int ldc
) {
    return xkblas_get()->syrk<xkblas_precision_t::££>(uplo, trans, n, k, alpha, A, lda, beta, C, ldc);
}
